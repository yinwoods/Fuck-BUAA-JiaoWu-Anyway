...
class Generate_schedule_table:
    ...
    def insert_null_courses(self, res):
        ...
        for item in res:
            ...
            if len(cur_day_courses) == 1:
                ...
                # 补充多节课之间的空闲课程
                for index, cur_course in enumerate(
                        cur_day_courses[1:], start=1):
                    if (int(cur_course['st_pd']) -
                            int(cur_day_courses[index - 1]['ed_pd']) > 1):
                        for pd in range(
                                int(cur_day_courses[index - 1]['ed_pd']) + 1,
                                int(cur_course['st_pd'])):
                            to_insert_courses.append(null_course(item, pd))
    ...
    def deal_conflict_courses(self, beautify_schedule_info):
        ...
        # 按照天、上课开始节数、结束节数依次排序
        beautify_schedule_info.sort(
            key=lambda x: (int(x['weekday']),
                           int(x['st_pd']), int(x['ed_pd'])))
        ...
        for index, cur_course in enumerate(
                beautify_schedule_info[1:], start=1):
            ...
            if self.is_conflict(cur_course, pre_course):
                ...
                res_schedule_info[-1]['st_ed_weeks'] += (
                        ' / ' + cur_course['st_ed_weeks'])
                res_schedule_info[-1]['st_ed_pds'] += (
                        ' / ' + cur_course['st_ed_pds'])
                res_schedule_info[-1]['ed_pd'] = max(
                        res_schedule_info[-1]['ed_pd'], cur_course['ed_pd'])
            else:
    ...
    def generate(self):
        ...
        for course in self.schedule_info:
            ...
            st_ed_weeks = re.compile(r'\d+周-\d+周').search(
                    course.course_time).group()
            (start_period, end_period) = re.compile(
                    r'(\d+)节-(\d+)节').search(course.course_time).groups()
            st_ed_pds = start_period + '节-' + end_period + '节'

            # 利用zip封装 keys values
            keys = ('name', 'weekday', 'st_pd', 'ed_pd',
                    'place', 'st_ed_weeks', 'st_ed_pds')
        ...
        beautify_schedule_info = self.deal_conflict_courses(
                beautify_schedule_info)

        # 创建一个dict，用以保存所有课程信息
        all_weekdays_dicts_list = []
        for i in range(1, 8):
            all_weekdays_dicts_list.append(
                    dict(zip(('weekday', 'courses'), (str(i), []))))
        ...
        all_weekdays_dicts_list = self.insert_null_courses(
                all_weekdays_dicts_list)
...
def main():
    param_str = [
        ...
        ('学术论文写作', '周5  15周-18周  5节-8节', 'A1028 (8系)')
    ]
